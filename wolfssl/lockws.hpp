...
lock_client::lock_client(std::string_view url){
    ...
    if(!error){
        ...
        else{ // unsupported websocket format
        ...
        if(!error){ // only continue if no error
            ...
            if(!error){ // only continue if no error
                ...
                // only continue if no error
                    ...
                    // we store the start index of the path from the supplied url - we search for the next forward slash after the protocol prefix, that is the start of the path in the supplied url string view
...
lock_client::lock_client(std::string_view url, in_addr* interface_address, char* interface_name){
    ...
    if(!error){
        ...
        // check if url is a wss:// endpoint. the wolfssl variant of lockclient supports only wss endpoints
...
bool lock_client::ping(){ // sends a ping on an established websocket connection
    ...
    if(!error){ // only continue if no error
        ...
        if(client_state == OPEN){ // continue if client is in open state 
            ...
            if(wolfSSL_write(c_ssl, send_data, i) <= 0){
        ...
        if(client_state == OPEN){ // only continue if lock client is in open state
            ...
            if( (rand_bytes[0] == (FIN_BIT_SET | RSV_BIT_UNSET_ALL | TEXT_FRAME)) || (rand_bytes[0] == (FIN_BIT_SET | RSV_BIT_UNSET_ALL | BINARY_FRAME)) ){ // this is the only frame of a text or binary frame data stream. We do not differentiate between text and binary frames since data copy happens the same way
                ...
                if(frame_data_len < static_data_array_length){ // static data array would be sufficient
                    ...
                    // we only attempt to read extra data if the frame data len is > 0 because some websocket servers may send an empty frame as acknowledgment of a message
                    if(frame_data_len > 0){
                ...
                if( !(++num_of_pings_received < ping_backlog) ){ // the ping backlog delays sending a pong frame till a number of pings specified by the ping backlog has been received
                    ...
                    // we only read the ping data if frame data len is > 0 because some websocket servers can send empty ping frames
                    if(frame_data_len > 0){
                ...
                // we only attempt reading extra bytes if frame data len is > 0
                if(frame_data_len > 0){
                ...
                // we declare a local cursor with which we read the received pong data into the upgrade_request_static variable because it isn't used by the program at this point
                char* loc_cursor = upgrade_request_static;

                // SIGPIPE signal is still blocked

                // we only attempt to read extra bytes if frame data len is > 0
                if(frame_data_len > 0){
...
int lock_client::connect_to_server(const char *hostname, const char *port, in_addr* interface_address, const char *interface_name){
    ...
    // we bind to the supplied interface name if any
    ...
    for(p = res; p != NULL; p = p->ai_next) {
        ...
        // Try to connect - we prepend the global resolution operator because the lockclient class has a connect function
    ...
    if(res != NULL) freeaddrinfo(res); // Free the addrinfo structure if non null
...
bool lock_client::close(unsigned short status_code){ // this closes an established websocket connection although the object itself still exists till it goes out of scope, the object can be connected to a different or the same websocket server using the connect function
    ...
    if(!error){ // only continue if no error
        ...
        if(client_state == OPEN){ // only continue if client is in open state
            ...
            // send the close frame
            wolfSSL_write(c_ssl, send_data, i);
